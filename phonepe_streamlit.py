...
if selected == "Explore Data":
    ...
    if Type == "Insurance":
        ...
        cursor.execute(f'''select state, year, quarter, sum(Transactional_amount) as Total_amount from map_insurance
                       where year= {selected_year} group by year, quarter, state order by Total_amount desc''')
        
        df = pd.DataFrame(cursor.fetchall(), columns=['State','Year','Quarter','Total_amount'])

        fig = px.icicle(df, path=[px.Constant(selected_year), 'Quarter', 'State'], 
                        values='Total_amount',
                        color='State',
